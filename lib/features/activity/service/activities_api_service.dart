...
final activitiesAPIServiceProvider = Provider<ActivitiesAPIService>((
  ref,
) {
...
class ActivitiesAPIService {
  ...
  Future<List<Activity>> getActivitiesForTrip(String tripId) async {
    try {
      ...
      activites.sort(
        (a, b) => a!.activityDate.getDateTime().compareTo(
          b!.activityDate.getDateTime(),
        ),
      );
  ...
  Future<void> deleteActivity(Activity activity) async {
    try {
      await Amplify.API
          .mutate(request: ModelMutations.delete(activity))
          .response;
    } on Exception catch (error) {
  ...
  Future<void> updateActivity(Activity updatedActivity) async {
    try {
      await Amplify.API
          .mutate(request: ModelMutations.update(updatedActivity))
          .response;
    } on Exception catch (error) {
